...
def _chapters():
    ...
    def c4(doc, num, label, blurb):
        ...
        doc.paragraph(
            "Every page lives at exactly one URL: a trailing-slash copy is 301-redirected "
            "to the slash-less canonical, and the sitemap never lists two URLs for the "
            "same page — so Google never reports duplicates.")
        doc.paragraph(
...
"platforms need no native key: they publish through the webhook "
        ...
        doc.paragraph(
            ...
            "(X, Pinterest, Facebook, LinkedIn, Instagram, Telegram, YouTube, "
            "Threads) gets "
        ...
        doc.paragraph(
            ...
            "id — posts your share card as a native photo), Threads (a Threads "
            "user token — text or image, via the Graph API ``me`` path) and "
            "YouTube (an OAuth "
...
"same read the live publisher does — Telegram getMe, Facebook Graph "
             "me, Instagram Graph business id, Threads me, LinkedIn /me and "
             "YouTube channels "
...
"webhook at POST /api/telegram/hook captures them in real time "
    ...
    def c14(doc, num, label, blurb):
        ...
        doc.text("trypstore.com", doc.margin_x + 18, y + 10, 10, (130, 120, 145))
        doc.uri(doc.margin_x, y, box_w, box_h, "https://trypstore.com")
        doc.spacer(30)
...
<p>Every page lives at exactly one URL: a <b>trailing-slash copy</b> (<code>/n/keto/</code>, <code>/sitemap.xml/</code>) is 301-redirected to the slash-less canonical, and the sitemap never lists two URLs for the same page. Google therefore never bisects your indexation or reports duplicates — one URL per page, one set of backlinks.</p>
...
<p><b>Nothing indexed yet? Run the 🩺 End-to-end doctor</b> on the Engines hub. In one click it fetches <code>robots.txt</code> and <code>sitemap.xml</code> the way a bot does, then samples real sitemap URLs and proves each one is indexable (200, self-canonical, no <code>noindex</code>), verifies the <b>IndexNow key file</b> that every engine fetches before it trusts a ping, reads each console's connection state, and checks whether social posts can actually be delivered. Each failed check carries the exact fix — and it states the hard truth that <b>IndexNow never reaches Google</b>, so a site that only pings IndexNow and never connects Search Console can rank on Bing/Yandex/DuckDuckGo but stay invisible on Google.</p>
...
<p>The <a class="tooltag" href="/admin/weeklydigest">💵 Weekly money digest</a> is the capstone loop on top: once a week (ISO weekday 1 · 09:00 UTC by default) every hot/converted subscriber of a niche that had a best-clicked winner <i>or</i> a real price drop gets ONE money email — a hero winner link (the single tracked hop the site counts), the week's real deals, a per-referrer re-enroll line, and <b>auto-pause</b> (on by default) that records quiet, dead niches and silently drops them from the per-week money set — so a tracked-link email is never wasted — with a <b>Resume</b> button to bring a niche back (it re-enters the set next cycle). The page shows the current winners/deals/referrers/gate, a <b>Dry run</b>, and a <b>Send now</b> for an instant push. One digest per niche per ISO week — the gate makes sure nobody is emailed twice.</p>
<p>That page also runs the <b>Money-step A/B (permanent matchup)</b>: the ONE tracked hop is the thing that earns, so each subscriber is pinned to a subject + hero-CTA variant forever (cohort split on first send via <code>weeklydigest.mv.*</code> pins), and the scoreboard counts every variant's sent / opened / clicked / CTR from the digest's own open pixel + tracked click (index 99). <b>Seed matchup on top money niche</b> drops two default variants onto your highest-clicking niche automatically; edit any niche's variants, park/enable them, delete losers, and the 🏆 crown marks the variant that finally won. Because a subscriber never re-randomizes, the matchup stays matched literally forever.</p>
...
<ul class="step-list">
<li><b>1 · Social router (n8n)</b> — set the <code>SOCIAL_WEBHOOK</code> env var to a real n8n host and import the router flow the repo ships in <code>ops/n8n/*.json</code>, so every kit fans out to the platforms with no native backend here. Zero-budget alternative: point it at this site's own router (<code>SOCIAL_WEBHOOK=https://&lt;your-site&gt;/api/social/webhook</code>) and paste platform keys on <a class="tooltag" href="/keys">🔑 Keys</a> (or set them as env vars — <code>PSTORE_TELEGRAM_TOKEN</code>+<code>PSTORE_TELEGRAM_CHAT</code>, <code>PSTORE_PINTEREST_TOKEN</code>, <code>PSTORE_INSTAGRAM_TOKEN</code>+<code>PSTORE_INSTAGRAM_IG_USER_ID</code>, <code>PSTORE_YOUTUBE_TOKEN</code>, <code>PSTORE_FACEBOOK_TOKEN</code>, <code>PSTORE_LINKEDIN_TOKEN</code>, X: <code>PSTORE_TWITTER_CLIENT_ID/_CLIENT_SECRET/_ACCESS_TOKEN/_ACCESS_TOKEN_SECRET</code>) so Telegram/Pinterest/etc. post natively with no third party.</li>
